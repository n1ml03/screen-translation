...
namespace ScreenTranslation
{
    ...
    public partial class SettingsWindow : Window
    {
        ...
        private void LoadSettingsFromMainWindow()
        {
            ...
            openAiRealtimeEndpointTextBox.Text = ConfigManager.Instance.GetOpenAiRealtimeEndpoint();
        ...
        private void UpdateServiceSpecificSettings(string selectedService)
        {
            try
            {
                ...
                // Show/hide prompt template and related controls
        ...
        private void ChatGptEndpointTextBox_TextChanged(object sender, TextChangedEventArgs e)
        {
            try
            {
                ...
                string endpoint = chatGptEndpointTextBox.Text.Trim();

                // Update the config
                ConfigManager.Instance.SetChatGptEndpoint(endpoint);
                Console.WriteLine("ChatGPT endpoint updated");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error updating ChatGPT endpoint: {ex.Message}");
            }
        }

        // ChatGPT Username changed
        private void ChatGptUsernameTextBox_TextChanged(object sender, TextChangedEventArgs e)
        {
            try
            {
                ...
                string username = chatGptUsernameTextBox.Text.Trim();

                // Update the config
                ConfigManager.Instance.SetChatGptUsername(username);
                Console.WriteLine("ChatGPT username updated");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error updating ChatGPT username: {ex.Message}");
            }
        }

        // ChatGPT Password changed
        private void ChatGptPasswordBox_PasswordChanged(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                string password = chatGptPasswordBox.Password;

                // Update the config
                ConfigManager.Instance.SetChatGptPassword(password);
                Console.WriteLine("ChatGPT password updated");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error updating ChatGPT password: {ex.Message}");
            }
        }

        // Endpoint TextBox KeyDown handler
        private void EndpointTextBox_KeyDown(object sender, System.Windows.Input.KeyEventArgs e)
        {
            if (e.Key == System.Windows.Input.Key.Enter)
            {
                // Focus next element or handle enter key
                ...
                ((System.Windows.Controls.TextBox)sender).MoveFocus(request);
            }
        }
        ...
        private void OpenAiRealtimeEndpointTextBox_TextChanged(object sender, TextChangedEventArgs e)
        {
            if (_isInitializing) return;
            ConfigManager.Instance.SetOpenAiRealtimeEndpoint(openAiRealtimeEndpointTextBox.Text.Trim());
        }
        ...
        private async void InstallServerButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                MessageBoxResult result = System.Windows.MessageBox.Show(
                    "Are you sure you want to install the environment for Translation Server?\n\n" +
                    "This process will create a Python virtual environment and install all required dependencies.\n" +
                    "This may take a long time and requires an internet connection.",
                ...
                if (result == MessageBoxResult.Yes)
                {
                    Console.WriteLine("Setting up environment for Translation Server...");

                    // Run setup using the standardized OcrServerManager method
                    await Task.Run(() =>
                    {
                        OcrServerManager.Instance.SetupOcrEnvironment("PaddleOCR");
                    });

                    Console.WriteLine("Translation Server environment setup completed");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error installing server: {ex.Message}");
                System.Windows.MessageBox.Show($"Error installing server: {ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
            finally
            {
        ...
        private async void StartServerButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                Console.WriteLine("Starting Translation Server...");

                // Start the server using OcrServerManager
                bool success = await OcrServerManager.Instance.StartOcrServerAsync("PaddleOCR");

                if (success)
                {
                    Console.WriteLine("Translation Server started successfully");
                    System.Windows.MessageBox.Show(
                        "Translation Server started successfully!\n\n" +
                        "You can now access the server from any device on your network.\n" +
                ...
                {
                    Console.WriteLine("Failed to start Translation Server");
                    System.Windows.MessageBox.Show(
                        "Failed to start Translation Server.\n\n" +
                        "Please make sure you have installed the server environment first.\n" +
            ...
            catch (Exception ex)
            {
                ...
                System.Windows.MessageBox.Show($"Error starting server: {ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
            finally
            {
                // Re-enable the button after the operation is complete
