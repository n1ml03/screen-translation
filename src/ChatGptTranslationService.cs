...
namespace STGameTranslation
{
    public class ChatGptTranslationService : ITranslationService
    {
        ...
        {
            string endpoint = ConfigManager.Instance.GetChatGptEndpoint();
            string currenServices = ConfigManager.Instance.GetCurrentTranslationService();
            try
            {
                // Get endpoint from config, use default model for compatibility
                string model = "gpt-4o-mini"; // Default model for endpoint compatibility

                // Validate we have an endpoint
                if (string.IsNullOrWhiteSpace(endpoint))
                {
                    Console.WriteLine("ChatGPT endpoint is missing. Please set it in the settings.");
                    return null;
                }
                ...
                {
                    Console.WriteLine($"Error calling endpoint: {response.StatusCode}");
