    ...
    public class Logic
    {
        ...
        public async void Init()
        {
            try
            {
                ...
                // Only connect to socket server if using PaddleOCR
        ...
        {
            // Only try to reconnect if we're using PaddleOCR
            if (MainWindow.Instance.GetSelectedOcrMethod() != "PaddleOCR")
            {
        ...
        {
            // If not connected and we're using PaddleOCR, start the reconnect timer
            if (!isConnected && MainWindow.Instance.GetSelectedOcrMethod() == "PaddleOCR")
            {
        ...
        public async void SendImageToServerOCR(string filePath)
        {
            ...
            try
            {
                // Check if we're using Windows OCR or PaddleOCR
                            ...
                            block.TryGetProperty("text", out JsonElement translatedTextElement))
                        {
                            ...
                            if (!string.IsNullOrEmpty(id) && !string.IsNullOrEmpty(translatedText))
                            {
                                ...
                                if (id == "999")
                                {
                                    ...
                                    string[] translatedParts = translatedText.Split("|||ST_SEPARATOR|||");
        ...
        public async Task TranslateTextObjectsAsync()
        {
            try
            {
                ...
                var combinedText = string.Join("|||ST_SEPARATOR|||", _textObjects.Select(obj => obj.Text));
