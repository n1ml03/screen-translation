...
namespace STGameTranslation
{
    public class OpenAIRealtimeAudioServiceWhisper
    {
        ...
        {
            string endpoint = ConfigManager.Instance.GetOpenAiRealtimeEndpoint();
            if (string.IsNullOrWhiteSpace(endpoint))
            {
                System.Windows.MessageBox.Show(
                    "OpenAI Realtime endpoint is not set. Please configure it in Settings.",
                    "Endpoint Missing"
                );
                return;
            }
            ...
            // Note: For custom endpoints, authorization may be handled differently
            // ws.Options.SetRequestHeader("Authorization", $"Bearer {apiKey}");
            ws.Options.SetRequestHeader("OpenAI-Beta", "realtime=v1");

            // Establish the websocket connection to the configured endpoint
